...
//            UIImageWriteToSavedPhotosAlbum(uiImage, self, #selector(imageSaveHandler(image:didFinishSavingWithError:contextInfo:)), nil)
    ...
    // @objc
    func startDetection(completionHandler: @escaping () -> Void) {
    ...
    //                        self.arView.scene.addAnchor(textAnchor, removeAfter: 3)
                            ...
                            let obj = SectionClassificationObject(direction: "wip", coord: points[i], distance: distanceAtXYPoint, classification: _classification)
                            objects.append(obj)
                            // print(objects.count)
